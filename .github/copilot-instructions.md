...
The suite is compute-bound, so `[profile.test]` in `Cargo.toml` builds it
optimised: every assertion and overflow check stays on and the whole run went
from about three minutes to **14 seconds** (400 unit tests 30 s to 2.5 s,
`tests/plates.rs` 97 s to 7.8 s). Do not turn that off to "get a real debug
build" — nothing here depends on unoptimised codegen.
...
- **A covered sample pinned to its plane between two followed ones is a JAB, and the sample-by-sample coverage answer will produce them.** Coverage is decided per sample, a sample is a fraction of a cell, and a bead grazing the cells that close over a hole flips for a step or two and back — so `sample` pins those to the plane while their neighbours follow the surface down. A flat nozzle cannot rear up and come straight back within its own underside; it smears the crest and the pass beside it comes down on what is left. In a preview they are dots around every hole. Measured on a user's coupon, `--bricks --zaa` wrote **26 of them, the worst 91 µm up and back over 46 µm of travel, against NONE for either transform alone** — bricking moves the visible wall inward, so its path meets those cells at a different angle and clips more of them. `Pass::follow_notches` gives a covered run under a bead wide the height of the sample before it, and `Pass::unjab` is the bound that cannot be evaded: no sample may stand above its neighbours more steeply than the climb, **including the first and last of a bead**, which is where the last one hid. Do NOT bound only the interior samples, and do NOT read the join height from `pending.samples` — `release` clears those the moment it writes them. Pinned by `plates::a_surface_around_holes_is_not_dotted_with_crests` over `tests/fixtures/coupon.gcode.zst`, the only fixture whose top surface has holes through it.
...
- **A followed bead must never fall faster than a slope can, and a CLIMB is
  ...
  held to a looser figure, but it is held: one layer height per half bead —
  twice the descent, and the steepest a bead can rise and still have the bead
  laid *beside* it come back down. What bounds a descent is the nozzle's flat
  underside plowing back through material it laid a bead width ago, and a climb
  lifts away from that material into a gap the extrusion is already metered
  for — so a climb bound of one layer per GRID CELL, tried once, produced a
  gentle fall and a sheer rise: **62 reversals of direction in 284 samples, the
  worst 3.70 mm per mm**. Do NOT give the climb the descent's own figure, and
  do NOT put the grid-cell bound back: the far edge of a strip is exactly
  where the ramp must reach half a layer for one layer's ramp to meet the next
  one's, and at the descent's figure a tread narrower than a bead was levelled
  outright and the staircase came back. Pinned by
  `zaa::tests::a_surface_never_drops_faster_than_a_slope_can` and
  `zaa::tests::a_climb_is_held_to_the_surface_rather_than_to_the_descent_limit`.
...
- **Gap fill and a thin wall are FILLERS: buffered with a wall, never one of
  ...
  layer of step on the outside of the part. Both take `geometry` like any other
  bead — a thin wall is plainly a bead of its own with a plane under it, and
  gap fill is laid into the valley between two beads and straddles whatever
  they did, so its plane is a coarser answer but still a measured one: the
  alternative is not "as sliced" but "metered for a whole layer over half a
  gap". The two go together — a filler that stops a column being capped has to
  be metered against the column it stopped, and that is also the one reason a
  filler may count as covering a raise below it.
...
- **A bead's nozzle charge is measured in INPUT order, not inferred from the next prime or the bead's size.** `Pass.input_withdrawn` and `Buffered.withdrawn` preserve the withdrawal before each move. Both output paths match that state at a bead; `matching_charge` reduces redundant primes and wipes without turning a wipe into an extrusion. Wipe paths survive even when their redundant pull becomes `E0`. Never clamp the model while writing the full pull, and never infer a deliberately dry taper from a small `E` or from a zero prime in processed output. The public `inner-outer-inner` fixture passed its old filament-balance check while containing a 3.0 mm excess prime and a bead missing 2.40 mm of filament. `Pass.stopped` keeps a height move's withdrawal reserved through the loop's prime. Added travel pulls still use the file's stated retraction length and minimum travel, measured from `wrote_at`. Count wipes along their XY paths, not just bare negative-E lines. `tests/nozzle` checks wipe preservation, excess primes, dry beads, primed travel and stops independently of filament totals; excess priming and starvation can cancel in the total. See the BrickLayers skill's measured-charge section and `reordered_primes_balance_without_double_priming_or_starving_small_beads`.
...
- **A wall laid beside support is held flat, and the test for "beside" is the support's own cells dilated by a bead.** Support is one bead wide and printed to break away, so a raise beside it stands proud of material nothing above a step could meet — measured on a user's tree-support slice, a raised wall scraped a tree tip every layer until it knocked the tree off the plate at layer 248. The survey records each layer's support beads in `Survey::support`, and `mark_columns` dilates them by two cells (`Cells::dilated(2)`: the nozzle's underside reaches half a bead past the wall and the bead reaches half a bead back) and caps any loop whose walk touches them. Pinned by `brick::tests::a_wall_beside_support_is_never_raised`.
- **A travel that crosses support may be LIFTED over it by the slicer's own hop but must never be RIDDEN DOWN onto it, and the ride is decided on the travel's real path, not the buffer's.** A reordered loop's travel can be a whole journey the slicer never wrote — the outer wall written first from the spiral hop over a tree tip, its travel then running from the tree to the wall. `Pass::clearance` walks the lead's steers moves from where the nozzle actually stands (`at_now`) and asks two questions, answered apart because they call for different action: does it cross a bead this pass raised (`laid`, clear to `laid_top`), and does it cross support (`survey.support`, clear to `plane + height`)? A raised-bead crossing owes a lift where the nozzle starts below the bead and refuses the ride where the loop's height is below it; a support crossing only ever refuses the ride — the slicer already travelled over the support at a height it chose, so nothing here lifts for it. Do NOT walk the lead off the buffer's own `.at`: a comment's `.at` is inherited from the input's frame, so the first non-steers line clobbers `at_now` and the walk starts from the input's position instead of the nozzle's — that left `over_support` false on the real file and the ride flattened the travel onto the tree. Pinned by `brick::tests::a_travel_crossing_support_is_not_ridden_down`.
- **A reordered travel is a journey, and the acceleration the slicer put in front of its short hop must not crawl it.** Bambu sets `M204 S250` before the 0.4 mm hop that reaches the outer wall, a tenth of the travel acceleration, metered for a fraction of a millimetre. Written first by the reorder, that hop becomes a 37 to 80 mm journey and inherits the `M204 S250`, so it crawls — measured on a user's tree-support slice at the F60000 the slicer metered it at. `write_loop` holds back any `M204` ahead of the first steers move when the loop is displaced (`Pass::displaced > 0`) and puts it out again right after the travel, so the journey runs at the file's own acceleration and the bead still gets the approach the slicer wrote for it. The line is moved, never dropped: the yardstick counts every `M204` in and out and must match. `M204` is read off the arena bytes, not the parser, which folds it into `Code::Other`. Pinned by `brick::tests::a_reordered_travel_runs_at_the_file_s_acceleration_not_the_approach_s`.
- **A retraction corbel adds must be one the reordering made necessary.** Every pull is gated on `Pass::displaced`: the nozzle is not where the slicer left it AND the travel from where it really is exceeds the file's own `retraction_minimum_travel`. A travel the slicer planned itself needs nothing — it decided, knowing its own geometry, that the hop was not worth closing. Asking only the length added **8997 pulls to a file with 13234, median travel 1.19 mm**, and a retraction is not free: it costs a stop, a gap where the bead restarts and a bite out of the filament. Do NOT retract before a bare `G1 Z` that the loop then draws from — the ooze goes into its own bead, and `primed_stops` counts only a stop the nozzle then travels away from. `Pass::write_held` obeys the same gate, against the travel's real length from where the nozzle stands (`hop(lead, wrote_at)`), not an unconditional pull: a held loop written beside a region that happens to end within a millimetre of it is no journey — measured on a user's tree-support slice, 252 of 422 held writes travelled under 1 mm and pulled 0.8 mm for it. Pinned by `brick::tests::a_held_loop_reached_within_a_millimetre_is_not_retracted_for` and `a_held_loop_reached_across_the_plate_is_retracted_for`.

- **`F` is MODAL, so every line corbel inserts hands its own rate to the beads behind it.** A retraction is written at the retraction rate and a height move at the Z rate, and a slicer states the print rate ONCE per loop — on a bare `G1 F` between the travel and the first bead. Corbel's prime lands after that line, so the whole loop prints at the retraction speed. Measured on a stock 1000-wall Bambu plate, **39446 mm of bead, 44% of the whole file, came out at F1800 where the slicer asked for F11054** — 30 mm/s instead of 184, and only on the reordered half of the wall, which is what makes it look random; across the 25 plates it was 30.9% to 66.2% of each file. Nothing that counts or sums can see it: the bead's own `E`, the filament total, the retraction count and even the RANGE of rates in the file are all untouched, because F1800 is a rate the file does use. `Pass.feedrate` is what the output is REALLY left in — `unprime` and `move_z` both set it — and `Pass.wanted_feed` is what the file asked for; `Pass::settle_feed` writes `G1 F` to close the gap. It is discharged LAZILY, at the next move that states no rate of its own, on all three write paths (`replay`, `ride`, `emit`) and after `settle_plane`, which inserts a height move of its own — so a run of inserts costs one line rather than one each, and a move that names its own rate costs none. Do NOT restore it eagerly inside `unprime`. Do NOT carry the rate on the `Loop` beside `width` and `marker`: the rate is restated after the travel that reaches each loop, so it is already pulled in with that loop's lead — measured byte-identical on all 25 fixtures. A HELD loop is the one exception: its travel was read under the hoisted head's rate (a Bambu hop's F60000) and is written after every rate since has overwritten `wanted_feed` — measured on a user's slice, held travels of 121.7 mm came out at F14301 against the F60000 the slicer metered them at. `Held.rate` captures `wanted_feed` right after the head replay and `write_held` restores it before each held write; pinned by `brick::tests::a_held_loop_s_travel_runs_at_the_rate_the_hop_set`. A spiral Z-hop that goes nowhere (`G3` naming `Z` and no `X`/`Y`, no `E`) and ends at the height the nozzle already stands at is pure ceremony — reordering replays two of them back to back over the support tree the slicer lifted off of once (measured: two full revolutions at 0.4 mm clearance every layer, a double tree knocked off the plate). `Pass::redundant_hop` drops only that case, keeping its `F` for the travel behind it; a lift or a descent is never dropped. Pinned by `brick::tests::a_reorder_writes_one_spiral_hop_not_two`.
`Ledger.seconds` is what sees this — time spent laying bead, which that bug multiplied by six on the walls it touched and which a deliberate slowdown moves by a few percent. Comparing the RATES instead cannot work now that a rate is brought down on purpose; `bead_feed` is one-sided for the same reason, and it was being measured on TRAVELS, which is why the suite called 66% of a plate clean.

- **More filament through the same nozzle in the same time is melt nobody has.** A raise gives a bead half a layer more material and a followed surface meters a stretch for the thicker gap it crosses, and both leave `F` exactly where the slicer put it — so the hot end is asked for up to **1.54×** the throughput. A slicer clamps every bead against `filament_max_volumetric_speed` and slows it down where it would go over, so on a real file there is no headroom to take: measured on a stock Bambu plate whose filament states 15 mm³/s, **98.64% of the input's bead path sits at exactly 15**, and `--bricks` then asked for **up to 23 mm³/s, 54% over, across 16% of the file**; `--zaa` asked **46.7%** over. Nothing is delivered, so the one bead that must carry a layer and a half to fill the gap under a raised column is the bead that comes out starved — the transform's benefit lost exactly where it was supposed to happen. `E` is NEVER touched for this: `Survey.melt_rate` is the ceiling and `brick::Pass::metered_rate` / `zaa::Pass::slowed` bring `F` down by `min(1, ceiling / new_flow)`, so the same filament is delivered over more time. **Per BEAD, never per loop.** The multiple of filament is one number for a whole loop, but what each bead already flows at is not — a slicer slows a bridge or an overhang right down and gives it a fatter bead, so that one segment sits at the ceiling while the rest of its wall has room to spare, and taking the tightest bead put the bridge's speed on the whole wall. Measured per bead, the bulk of a real plate is slowed to **97-100%** of its own rate (the flow multiplier) and only the climbing beads reach **65%** (1/1.54), for **+4.4% to +4.7%** of the time spent laying bead. `settle_feed` must NEVER run in front of a bead: a bead settles its own rate, and a resume written ahead of one hands back the file's rate right after this pass decided the bead cannot have it — which made the rates ALTERNATE along a wall, every second bead unthrottled. The ceiling is PER TOOL. A plate printing two materials pins each to its own limit and they can be 3x apart — measured on a user's dual-nozzle H2D plate stating `filament_max_volumetric_speed = 8,18,18,25,25`, **T0 peaks at exactly 8.00 mm³/s over 282 m of path and T3 at exactly 25.00 over 145 m**. One ceiling for the whole file takes the higher and lets the PA12 run to 12.3, 54% over its own limit. `Survey.melt_rate` is a slot, `Survey::melt_at(tool)` the lookup, and `scan::tool_change` reads a `T` line — only an index under `MAX_TOOLS`, since a slicer brackets a change with codes of its own (`T1000`, `T1001`). Each slot's figure is the file's STATED one (`filament_max_volumetric_speed` ÷ the filament's section) floored by the fastest rate that slot's own beads reach, so a rate the slicer already demanded may never be called impossible, and where nothing is stated the measured peak stands alone (`support.gcode.zst` is that case, and unthrottled it ran 47.7% over). Do NOT take the slowest slot: that is what the yardstick did, and it called `support` 1.9% over when corbel was right. Measure it over EVERY bead, not just the walls: a top surface runs faster than a perimeter and `zaa` re-meters one, so a ceiling read off the walls alone slowed a surface bead to **5116 mm/min against the 9000 the file asked for**.

- **A region marker is MODAL, and a layer change does not end it.** A slicer states `; FEATURE:` once and keeps printing; one that means to change it says so. Bambu prints a thin tapering wall straight through a layer change without re-declaring the region at all — measured on a user's dual-nozzle plate, **20 of its 274 layers laid 6895 mm that way**. Read as no region, those beads are left out of the survey's footprint, so the layer above appears to cover nothing: `uncovered` for the layer below jumped to **1807 cells against a true 60**, which capped essentially the whole layer to 0.51x flow — a visible dark line right round the part — and bricking stopped above it, 4166 loops found instead of 4330. Do NOT reset `self.feature` at a layer marker in `scan.rs`, `brick.rs`, `zaa.rs` or `zaa/scout.rs`; all four have to read it the same way or one pass draws a cell another asks about. The bead a slicer carries across the break IS the wall it last named, so it is metered as one. Pinned by `brick::tests::a_layer_change_does_not_end_the_region_it_interrupts`.

- **A tool change is a wall a loop may never cross.** A raised loop waits for the end of its layer, and on a plate printing two materials the tool changes in the MIDDLE of one — so a loop still waiting is written after the change and laid in the other filament. Measured on a user's dual-nozzle plate, **41818 mm of bead crossed from T0 to T3**, which prints PA12 walls in ABS. `brick::Pass::feed` flushes the buffer and writes the held loops before the `T` goes out. Only where a loop could actually cross it: a slicer also drops a `T` between a layer's `G1 Z` and its wall, and that one is part of the tail `keep` holds so the first loop has a move to ride — flushing there costs every raise its carrier. Pinned by `brick::tests::a_loop_is_never_written_under_another_tool`. Do NOT deliver the rate as a `G1 F` line in front of the bead: `zaa` reorders lines around a bead it re-meters, and the bead then runs at whatever it finds — `Line::write_moved_at`/`write_e_at` put the rate ON the move, adding the word where it had none, and ONLY where it differs from what the line would otherwise run at, so a file with melt to spare comes through byte for byte. A `G92`, a travel and a wipe are never given one. Do NOT measure the flow from the INPUT geometry: the visible wall is moved sideways and a corner does not change length by the same proportion as its ring, which left a bead 1.9% over on the cone; and measure the span at the THREE DECIMALS it is written to, since a micron of rounding on a half-millimetre piece is a fifth of a percent of its flow. Meter the FILAMENT at the five decimals it is written to as well: metering the unrounded product under-throttles a bead whose written `E` rounds up — measured on the cone plate, a bead metered at 0.0027551 mm came out as E0.00276 and melted 0.18% over the file's own ceiling. `MELT_GAUGE` is 0.05 mm, not 0.5: a slicer's corner segments run 0.1–0.5 mm, and at half a millimetre they skipped the throttle their neighbours took, flipping the rate at every corner (F10565 on an edge against F13265 on its own corner beads, both at the same flow). The yardstick in `tests/nozzle` imports the crate's `MELT_GAUGE`, so both sides count the same beads. Both sides are pinned: `Ledger.peak_melt` over all 25 plates in both modes, `brick::tests::only_the_bead_that_goes_over_is_slowed`, `brick::tests::a_short_corner_bead_is_slowed_like_the_edge_beside_it`, and `bead_feed.0` against `MOST_SLOWED` — slowing a bead divides its rate by the filament it was given and the most any bead is given is 1.54, so anything deeper is a rate that belongs to a height move or a retraction, which reads as a bead at a thirtieth of the flow and draws a line across the part.
