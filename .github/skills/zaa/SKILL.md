...
- **Nor does one ever fall faster than a slope can — and a climb is bounded by something else entirely.** The surface is at its highest exactly where the layer above begins — that is what makes one layer's ramp meet the next one's — and a bead carrying on under the layer above has to be back on its plane, so the constraint has a half-layer step in it with one grid cell to do it in. Left raw that is not a path a nozzle can take: on the 60 mm cap **886 of 2207 written moves changed height faster than one in two**, the worst by 0.1 mm over 0.023 mm of travel. `Pass::ease` spreads the descent out ahead of the edge instead, at no more than one layer height per bead width — the steepest a surface can fall and still be a slope rather than a wall. It only ever **lowers** a sample and never touches a covered one, so a bead under the layer above still sits exactly on its plane and the visible wall keeps its ceiling. After it, all four real parts have **zero** moves steeper than one in two. Pinned by `zaa::tests::a_surface_never_drops_faster_than_a_slope_can`, which reads 4.000 against a limit of 0.444 with it switched off.
  ...
  A **climb** is held to a looser figure, but it is held: one layer height per
  **half bead** — twice the descent's figure, and the steepest a bead can rise
  and still have the bead laid *beside* it come back down. What bounds a
  descent is the nozzle's own flat underside plowing back through material it
  laid a bead width ago; climbing, it lifts away from that material into a gap
  the extrusion is already metered for. A climb bound of one layer height per
  **grid cell** was tried first and produced a gentle fall and a sheer rise:
  **62 reversals of direction in 284 top-surface samples, 28% of the path
  steeper than one layer per bead width and the worst 3.70 mm per mm** — eight
  times what a descent is allowed. Held to the descent's own figure instead it
  flattened a great deal: the far edge of a strip is exactly where the ramp
  has to reach half a layer for one layer's ramp to meet the next one's — so a
  tread narrower than a bead was levelled outright and the staircase came
  back. Pinned by `zaa::tests::a_climb_is_held_to_the_surface_rather_than_to_the_descent_limit`,
  whose climb figure is what this describes, not
  `a_surface_never_drops_faster_than_a_slope_can`, which only measures falls.
...
- **The chamfer kernel is faster written as it is than hand-unrolled.** Replacing the closure with eight straight-line reads at fixed offsets from `at` — no casts, no comparisons, just the index arithmetic — came out **86% slower** (14.9 s to 27.8 s), and so did a version off three row slices, and putting the direction in a `const` parameter did not help, and neither did reducing the eight `min`s as a tree instead of a chain (6.4 s to 7.1 s). The four comparisons per neighbour let the compiler prove the index is in range and drop the bounds check with it. Do not "optimise" this loop again without measuring, and measure it against a whole real file.
  **Padding the window does not remove the check either, and costs 44%.** A ring of `UNREACHED` cells around the window is exactly equivalent — the ring is never a source, and `UNREACHED` saturates back to itself under the costs — so the four comparisons per neighbour can go. What replaces them is a slice index the compiler still cannot prove, now behind a branch rather than a conditional move, and the offsets stop being constants the moment they are written as `dy * stride + dx`. Measured on the same 7.4 MB part: **16.2 s of CPU against 23.4 s**, wall clock 7.1 s against 10.6 s, byte-identical either way. Safe Rust cannot express an unchecked index, and that is the whole of why this loop is left alone.
...
- **Banding a band with a halo is exact but does not pay, and the halo is why.** A row band computed on its own gives the whole window's answer if the band is padded by the furthest a source can influence it — `CAP / 5` rows, since a chamfer step that moves a cell costs at least 5 and one that moves diagonally costs 7 over √2. That is `carried / cell` rows, which is not a small number: on a 7.4 MB part at 0.0591 mm cells and 0.08 mm layers it is **242 rows of a 1960-row window**, so four bands carry 2.5× the work for 4 lanes and sixteen bands carry 5× for 16. The resolution is what makes this circular — the halo is the reach measured in cells, and the grid is spent on whatever span the part has — so a part barely wider than its own reach can only ever be one band. Measured: not worth the memory it needs (a private buffer per band, since the halo rows of two bands are the same rows and both write them).

### The overhang underside has no surface, and is answered before any of this

A layer whose own outline encloses everything the layer below encloses is the underside of an overhang. Nothing about it faces upward: the layer beneath stands at least as far out as this one on every cell, so `down − out` — the whole of `sloped` — cannot come out positive anywhere, and the field is the plane. `Builder::build` returns right after the mouths, before the transforms.

**The proof is where the two distances are measured from, and it is what makes this exact rather than a heuristic.** Both `to_mine` and `to_under` are distances to the *outside* of a layer, and the flood that sorts inside from out seeds the window's edge — no bead and no mouth reaches it — so the edge ring is a source of both, each is finite everywhere, and nesting the two sets of *inside* nests the two sets of *sources* the other way round. A chamfer transform loses distance when it is given more sources, pass by pass, so `to_under ≤ to_mine` in every cell. Note what that argument rules out beside it: a cell whose `to_under` is `UNREACHED` reads `sloped` as 1.0, and `UNREACHED` is only reachable where `to_mine` is too, which the skip test above it already catches.

Worth having or not depends on the part, and it is worth measuring before claiming it. On a 7.4 MB, 313-layer part, 313 layers build a window and **one** of them moves a bead — the early-out still fires only on some of the rest, and saved 0.37 s of 7.1 s on that file. The mirror of it is the cone, and the two are pinned together by `the_underside_of_an_overhang_is_not_followed`: nested the wrong way round, the second case stops being followed and a staircase nothing removes comes back.
