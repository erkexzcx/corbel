...
**A path is found through its SAMPLES, and that is what sizes the window a query sweeps — never the reach.** A cell holds a path when one of the path's samples lands in it, and `Ground::add` lays those every `CELL / 2`; the nearest sample to the nearest point of a path is therefore half that step away, so a path that can win is registered no further than `(reach + CELL / 4) / CELL` cells away — one cell either way at every reach this module is asked for, so the window is three cells square. Five is the trap: a path registered a couple of cells out *can* have its nearest point nearby (it loops back), so widening the window looks harmless and is only slower. Widening it is what this module did, and the sweep was **nine tenths of `brick`'s whole time** — measured on a real 3.7 MB slice, `--bricks` took 22.2 s with the five-cell window and 4.2 s with three, byte-identical output. The other half of that is that a path registered in several of the window's cells was measured once per cell; `Ground::seen`/`generation` stamps it, and `Ground::gathered` keeps the window a query already swept for the next sample that lands in the same cell — a bead's lateral samples are a tenth of a cell apart and almost always share one. Pinned by `a_path_reaches_a_query_standing_in_the_cell_beside_it` and `a_query_in_another_cell_is_measured_against_that_cell_s_own_paths`.
...
**Bricking preserves each bead's original filament-per-second rate.** `Pass::bead_limit` takes the lower of that input move's throughput and the active tool's melt ceiling. Both `metered_rate` and `split_bead` use it, with no short-move exemption. Extra material per millimetre is delivered by slowing `F`, never by increasing local throughput; reduced material never increases head speed. This replaces the previous global-ceiling-only policy, which allowed increases wherever the profile claimed headroom. Meter using the actual written path and rounded M82/M83 extrusion delta. Pinned by `extra_material_slows_each_bead_even_with_unlimited_melt_headroom` and the split-circle throughput test.
...
**The held write is ordered STACK BY STACK, never by height across the layer.** A raised loop waits for the end of its layer, and while that wait was one queue sorted by `rise_of` the nozzle toured every feature twice: the flat loops of every island, then the raised ones. Measured on two private slices whose islands stand 72 to 83 mm apart (`--bricks` only), journeys across the gap went from the slicer's own **28 and 27 to 51 and 51**, covering 3.51 m and 3.15 m against 6.28 m and 6.47 m. `Pass::held_stacks` groups the held loops into stacks — a contour, or several whose extents come within `MAX_LOOP_GAP` — and `write_held` visits them from wherever the nozzle stands, lowest first within each. Height is owed only between loops that run beside each other, and `MAX_LOOP_GAP` is five times the nozzle's own reach, so two stacks owe each other nothing and any of them may be written next. **Every flat loop is still written before any raised one** — this changes the order among the held loops and never which loops are held, which is why `tests/collision.rs` stays green. Afterwards the same two files read **33 and 29** journeys, 3.92 m and 3.09 m. Pinned by `brick::tests::two_islands_a_plate_apart_are_not_toured_twice_a_layer` (15 journeys against the slicer's 9 with one queue, 10 with stacks).

**A lead may not take the nozzle below what this pass has already raised on this layer.** The slicer's own descent was written for a nozzle standing where it hopped from, and a reorder can leave it standing on a raised bead of the same layer instead: measured on the `layer-max` plate, a travel plus its loop's descent ran 4 um under beads standing 20 um higher, which the plates hold to **0 um**, so `plates::a_layer_deeper_than_two_thirds_of_the_nozzle` fails without the rule. A lead line that would descend below the raised top while the lead crosses it carries that height instead — only a line this pass has not moved, since a moved line is written where the visible wall was taken to — and the loop's own raise or descent settles the height once the lead is over.

**A journey the reorder created must clear the layer by the file's own lift, and what earns one is what the travel CROSSES rather than how long it is.** The slicer lifts before it crosses a part and puts the nozzle down on the far side. A travel that is a journey only because this pass moved the loop still carries the lead of the short hop it used to be, which names no lift at all — so it crosses the part at bead height. Measured on a user's slice, a **102 mm island crossing ran 0.040 mm over the layer**, the height of the raised beads it passed over, where the slicer's own crossings of the same gap ran 0.320 and 0.400 mm over it. `Survey.travel_lift` reads `z_hop`/`retract_lift` per filament slot and `Pass::journey_lift` folds it into the floor the lead has to clear, which puts the sequence back to the slicer's own: retract, lift, travel, descend, prime, with the nozzle empty for all of it.

**Gating it on the file's `retraction_minimum_travel` — the gate every PULL this pass adds uses — was wrong, and a print proved it.** A pull costs filament and a stop; a lift is a `G1 Z` of its own, which is a dead stop and nothing else. On a part whose two ends stand 78 mm apart, the length gate put **184 stops on the end the slicer made 17 on**, and the user printed it: that end came out worse than with no lift at all, while the other end — whose travels really do cross the part — came out visibly better. The gate is geometric instead: the lead crosses a bead this pass raised (`clearance`'s own answer), or the loop is written below what this layer has already laid. On the same file that is **38 stops against 202**, with the crossings that run within 0.1 mm of the layer down from 25 to 3 and the distance they cover from 3561 mm to 548 mm.

It must not fire on a loop the slicer placed itself: a lead that already commands a height above the lift is the slicer's own hop, replayed. And "a lead with nothing in front of it" is NOT the test for a displaced loop — a held loop opens a buffer of its own, but so does a region's first loop, and reading `lead == 0` as displaced put a lift in front of every region's first loop and stamped 40 raises into a plate that bricks nothing. `Pass.holding_back` is set only around the held write. Pinned by `plates::a_solid_fill_that_does_not_stack_is_left_alone`.

**The order is by the LOWEST height a strand lays, not by the raise it took.** What the order exists for is the nozzle's underside, and a strand the layer above holds over only part of its length lays beads at BOTH heights — so the raise it took says nothing about what can be plowed. Measured on a user's 0-wall slice, two such strands written raise-first laid **104 beads 117 µm under material already standing within the nozzle's reach**, against none anywhere in the file the slicer wrote; the strands there cross rather than nest, so the coverage answer alternates bead by bead along one of them. `Loop::grounded` records that a strand lays at least one bead on its plane, and `Pass::lowest_of` — `0` where it does, the loop's rise where it does not — is what both `Pass::order` and `Pass::held_stacks` sort by, with `rise_of` as the tie-break. This is a refinement of the rule that was always there rather than a new one: lowest first, measured at the bead that can actually be plowed. Pinned by `brick::tests::a_strand_with_flat_beads_is_laid_before_the_raised_one_beside_it`, which reports 15 plowed beads with `rise_of` as the sole key and none with this.

Flattening the short raised runs instead was tried and is worse on both counts: it left the plows exactly where they were (104 against 103), because they are between two strands rather than within one, and it cost **57 of 1103 raised loops**.
...
For a repeatable stop on a particular layer, run `cargo run --release --example audit-extrusion -- contact <input.gcode> <output.gcode> --layer 13` (one-based layer number). This checks printed paths against all earlier deposited paths inside the bore-plus-half-bead reach and reports the highest commanded filament feed rates, including pieces shorter than the gap screen's 0.5 mm minimum. Its synthetic control deliberately prints beside higher material. Contact and commanded throughput do not measure motor force, actual nozzle temperature, filament condition or the printer's larger physical tip; retain the exact HMS error code when investigating an overload.

`throughput <input.gcode> <output.gcode>` checks every matched printing piece against its original move's extrusion rate and head speed, allowing only the written feedrate's rounding precision. `extrusion <before.gcode> <after.gcode>` requires identical printing geometry, order and filament amounts while allowing feedrates to change. Use both to verify a speed-only change, alongside `nozzle` for retractions and pressure state.

`layers <input.gcode> <output.gcode>` checks layer-marker counts, the independent `M73 L` sequence, the set of layers with printing moves, output heights against input planes, and survey heights against measured input increments. `--layer N` restricts printed detail, not the checks. Positive controls remove a layer, omit its Z advance and corrupt its progress number. This detects missing commanded layers, not a printer mechanically failing to execute a valid Z move. For repeatable stops, map `M73 P` to the actual layer before assuming percentage means layer count.
