...
**A journey the reorder created must clear the layer by the file's own lift.** The slicer lifts before it crosses a part and puts the nozzle down on the far side. A travel that is a journey only because this pass moved the loop still carries the lead of the short hop it used to be, which names no lift at all — so it crosses both islands at bead height. Measured on a user's slice, a **102 mm island crossing ran 0.040 mm over the layer**, the height of the raised beads it passed over, where the slicer's own crossings of the same gap ran 0.320 and 0.400 mm over it. `Survey.travel_lift` reads `z_hop`/`retract_lift` per filament slot and `Pass::journey_lift` folds it into the floor the lead has to clear, which puts the sequence back to the slicer's own: retract, lift, travel, descend, prime — the nozzle empty for all of it, so the added `G1 Z` is not a primed stop. Afterwards both private files cross at **0.206 mm and above, against the input's own 0.206 and 0.247**, and none runs within 0.1 mm of the layer.

Two things it must NOT do. It may not fire on a loop the slicer placed itself: a lead that already commands a height above the lift is the slicer's own hop, replayed. And "a lead with nothing in front of it" is NOT the test for a displaced loop — a held loop opens a buffer of its own, but so does a region's first loop, and reading `lead == 0` as displaced put a lift in front of every region's first loop and stamped 40 raises into a plate that bricks nothing. `Pass.holding_back` is set only around the held write, which is the one place the distinction is real. Pinned by `plates::a_solid_fill_that_does_not_stack_is_left_alone`.
